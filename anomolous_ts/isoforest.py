...
if __name__ == "__main__":
    ...
    # Combine components
    time_series = base_signal + trend + noise + anomalies
    ...
    print("\nExample 1: Basic Usage")
    detector = TimeSeriesIsolationForest(window_size=5)
    anomalies = detector.detect(ts)
    print(f"Found {anomalies.sum()} anomalies")
    print("\nExample anomalies:")
    print(ts[anomalies].head())

    # Example 2: Multivariate time series with missing values
    print("\nExample 2: Multivariate Time Series")
    # Create a second time series with different patterns and some missing values
    ts2 = ts.shift(24) + np.random.normal(0, 1, n_points)
    # Introduce some missing values
    ts2[np.random.choice(n_points, 50)] = np.nan
    df = pd.DataFrame({'series1': ts, 'series2': ts2})

    detector = TimeSeriesIsolationForest(
        window_size=10,
        n_estimators=200,
        seasonal_period=24,  # Daily seasonality
        imputation_method='linear'  # Use linear interpolation for missing values
    )
    multivariate_anomalies = detector.detect(df)
    print("\nAnomalies in multivariate data:")
    print(multivariate_anomalies.sum())

    # Example 3: With different imputation methods
    print("\nExample 3: Testing Different Imputation Methods")
    for method in ['linear', 'forward', 'backward', 'mean', 'median']:
        detector = TimeSeriesIsolationForest(
            window_size=12,
            seasonal_period=24,
            contamination=0.05,
        ...
        print(f"Found anomalies: {anomalies.sum().sum()}")